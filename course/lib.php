...
function course_module_bulk_update_calendar_events($modulename, $courseid = 0) {
    ...
    if (!empty($courseconfig->completioneventsmaxtime_enabled)) {
        $date = new \DateTime('now', core_date::get_user_timezone_object(99));
        $completionmaxtime = $date->getTimestamp() - $courseconfig->completioneventsmaxtime;
    }
    // Configuration to define if only visible events should be updated.
    if (!empty($courseconfig->completioneventsvisible)) {
        $completionvisible = $courseconfig->completioneventsvisible;
    }
