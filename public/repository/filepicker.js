       ...
       classnamecallback : function(node) {
        ...
        setup_toolbar: function() {
            ...
            toolbar.one('.fp-tb-uploadfile').one('a,button').on('click', function (e) {
                ...
                var bodyHtml = '<div class="dropzone-container"></div>';
                var draftidRef = { value: null }; // store draftid between uploads
                var uploadurl = this.active_repo.uploadurl;

                require(['core/modal_factory', 'core/modal_events', 'core/dropzone', 'core/notification', 'core/str'],
                    function (ModalFactory, ModalEvents, DropZone, Notification, Str) {
                    ...
                    // Hande file upload to save it as draft.
                    ...
                    function renderUploadedFile(modal, response) {
                        const $body = modal.getBody();
                        let $fileList = $body.find('.uploaded-files ul');

                        if ($fileList.length === 0) {
                             // Load the string asynchronously
                            Str.get_string('attachedfiles', 'repository').then(function(label) {
                                $body.append(
                                    '<br/><div class="uploaded-files">' +
                                        '<h5>' + label + '</h5>' +
                                        '<ul></ul>' +
                                    '</div>'
                                );

                                $fileList = $body.find('.uploaded-files ul');
