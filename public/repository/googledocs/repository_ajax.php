...
$repo = repository::get_repository_by_id($repoid, $contextid, $repooptions);
if (!$repo) {
...
switch ($action) {
    case 'upload':
        // Safe the files in the draft area.
        $draftid = !empty($itemid) ? $itemid : file_get_unused_draft_itemid();

        // Save the file into draft area.
        ...
        // Delete old if exists and create the new one.
    ...
    case 'commit':
        ...
        if (empty($itemid)) {
            die(json_encode(['error' => get_string('error', 'repository')]));
        }

        // Grab files from draft area.
